...
def test_sparse_predictions():
    ...
    Y0 = np.sin(X0).dot(A) + 1e-3 * (np.random.rand(5, k) - 0.5)  # Just something smooth
    smgpr = SMGPR(X0, Y0, num_induced_points=2)
    ...
    np.testing.assert_allclose(M, M_mat, rtol=1e-4)
